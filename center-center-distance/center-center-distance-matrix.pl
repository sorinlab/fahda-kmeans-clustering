...
# OUTPUT
	...
	$output = $output."_clusterDistance.txt";

	open (OUTPUT, ">$output") or die "Cannot write to output file $output. $!\n";
	foreach my $cluster (@clusterIDs) { print OUTPUT "\t$cluster"};
	print OUTPUT "\n";

	for (my $i=0; $i < scalar @clusterIDs; $i++) {
		print OUTPUT "$clusterIDs[$i]\t";
		# print filler for lower half of the matrix
		for (my $k = 0; $k <= $i; $k++) {
			print OUTPUT "-\t";
		}
