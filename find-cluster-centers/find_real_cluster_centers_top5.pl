...
# INPUT   :
# OUTPUT  :
# PURPOSE : Find the top 5 structures that are closest to the mean center for a
#           given cluster.
# ==============================================================================
# SAMPLE INPUT FILE IS SHOWN AT THE END OF THIS FILE

$usage = "perl script.pl  -i input.txt  [--top 5] -v  vector_length  -o output.txt";

#-----------------------GET ARGUMENTS-------------------------------------------
	$input        = "";
	$vectorLength = "";
	$topResults   = 5; # The number of top results that are closest to a center
	$output       = "";
	...
	for (my $i = 0; $i <= $#ARGV; $i++)
	{
		...
		if ($flag eq "--top") { $i++; $topResults = $ARGV[$i]; next; }
		if ($flag eq "-v") { $i++; $vectorLength = $ARGV[$i]; next; }
	...
	print "Input: $input\n";
	print "Number of best results per cluster: $topResults\n";
